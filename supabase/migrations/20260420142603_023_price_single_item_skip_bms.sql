/*
  # Add BMS Skip to price_single_boq_item

  ## Change
  After checking for manual overrides and zero-quantity items, add an early
  exit for BMS items. These require manual pricing via the BMS calculator.
  The item is left as 'pending' so it shows up in the unpriced list.
*/
...
BEGIN
  ...
  -- BMS items must be priced manually via the BMS calculator
  IF is_bms_item(v_item.description) THEN
    RETURN jsonb_build_object('success', false, 'reason', 'bms_manual_required', 'skipped', true);
  END IF;
