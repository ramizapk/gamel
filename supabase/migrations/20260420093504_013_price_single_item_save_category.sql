/*
  # Update price_single_boq_item to save category from rate library

  When a BOQ item is priced and matched to a rate library item,
  the category from the library item is now saved to boq_items.category.
  This allows the BOQ table to display the category for each priced item.
*/
...
BEGIN
  ...
  INTO v_best_lib_id, v_best_rate, v_best_cat, v_best_conf
